        ...
        viewModelScope.launch {
            ...
            try {
                ...
                tingRepository.getBooks().onSuccess { books ->
                    // Fetch favorites and merge - favorites API returns full book objects with id field
                    val favIds = mutableSetOf<String>()
                    tingRepository.getFavorites().onSuccess { favs -> favs.forEach { favIds.add(it.id) } }
    ...
    fun loadAndPlayAudiobook(bookId: String) {
        viewModelScope.launch {
            try {
                ...
                // Sync favorites from server - favorites API returns full book objects with id field
                tingRepository.getFavorites().onSuccess { favs ->
                    _favoriteBookIds.value = favs.map { it.id }.toSet()
                    _allBooks.value = _allBooks.value.map { it.copy(isFavorite = _favoriteBookIds.value.contains(it.id)) }
                }
